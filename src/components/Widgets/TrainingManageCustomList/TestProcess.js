...
class TestProcess extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      !_.isEqual(this.questionsListTestProcessTotalNum, nextProps.courseTestProcess.totalNum)
      || !_.isEqual(this.questionsListTestProcessTotalScore, nextProps.courseTestProcess.totalScore)
      || !_.isEqual(this.exerciseDOList, nextProps.courseTestProcess.exerciseDOList)
    ) {
      this.props.dispatch({
        type: 'createExamPaper/saveTotalValue',
        payload: {
  ...
  componentWillUnmount() {
    this.props.dispatch({
      type: 'createExamPaper/resetState'
    })
  }
  computeQuestionsListTestProcess = (listMap) => {
    ...
    for (const [key, value] of listMap) {
      ...
      // 总分
      const score = getFieldValue(`questionsScore${key}`) || 0;
  ...
  renderAddQuestionButton = (totalNum, id) => {
    return (
      <Tooltip title={totalNum >= 200 ? '最多添加200道试题，请删除部分后再添加' : ''}>
        <Button disabled={totalNum >= 200} onClick={() => this.props.onChooseTest(id, totalNum)} type="primary" ghost>添加试题</Button>
      </Tooltip>
    );
  }
  ...
  render () {
    const { courseTestProcess, /* allQuestionsOpenMap,*/ form, dataSource } = this.props;
    const { allQuestionsOpenMap } = this.state;
    const { getFieldDecorator, getFieldValue } = form;
    
    const { questionsScoreMap, checkedQuestionItemMap, questionsListMap, } = courseTestProcess;
    // 获取所有试题
    ...
    return (
      <div className={this.state.switchPannel ? `${styles.createCourseItemSprite} ${styles.createCourseContent} ${styles.createCourseItemSpriteClosePannel}`: `${styles.createCourseItemSprite} ${styles.createCourseContent}`} style={{ marginBottom: 0 }}>
        ...
        <div className={styles.createCourseTitle} style={{ marginBottom: 0 }}>
          <LinkSprite switchPannel={this.switchPannelHandler} deletePannel={(linkTabName) => {
            this.props.dispatch({
              type: 'createExamPaper/resetState'
            })
            this.props.deletePannel(linkTabName);
...
function select(state) {
  ...
  return { courseTestProcess: createExamPaper };
}
...
export default connect(select, actions)(TestProcess);
