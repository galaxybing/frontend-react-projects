...
import { formatOption, strOrNumConvert } from '../../utils/util';
import * as Service from '../../services/common/common';
import styles from './questionsModal.css';
import QuestionAttributeForm from '../Widgets/QuestionsManage/QuestionAttributeForm';
...
class QuerstionModal extends Component {
  constructor(props) {
    ...
    this.uuid = 0;
    Service.getLevelList().then((res) => {
      if (res.data.success) {
        let levelSelectAll = false;
        if (!this.isCreateModal) {
          if (currentQuestion.level && currentQuestion.level.split(',').length === res.data.data.length) {
            levelSelectAll = true;
          }
        }
        this.setState({ levelList: res.data.data, levelSelectAll });
      } else {
  ...
  componentWillMount() {
    ...
    if (this.isCreateModal) {
      ...
      const { answer, level, exerciseItemDOList } = currentQuestion;
      ...
      if (level) {
        ...
        levelArr.map((item) => {
          levelClickFlag.set(item, true);
        });
      }
      this.setState({ questionClickFlag, levelClickFlag });
    }
  }
  onTypeChange = (e) => {
    this.setState({
  ...
  levelClick = (value) => {
    const { levelClickFlag } = this.state;
    if (levelClickFlag.get(value)) {
      levelClickFlag.set(value, false);
      this.setState({ levelClickFlag, levelSelectAll: false });
    } else {
      levelClickFlag.set(value, true);
      let allFlag = true;
      if (levelClickFlag.size !== this.state.levelList.length) {
        allFlag = false;
      }
      for (const level of levelClickFlag.values()) {
        if (level === false) {
          allFlag = false;
          break;
        }
      }
      this.setState({ levelClickFlag, levelSelectAll: allFlag }, () => {
        const { setFieldsValue } = this.props.form;
        const ansLevel = this.strMapToArr(levelClickFlag).join('');
        setFieldsValue({ ansLevel });
      });
    }
  }
  levelAllClick = () => {
    const { levelClickFlag, levelSelectAll } = this.state;
    ...
    this.state.levelList.map((item) => {
      levelClickFlag.set(item.code, selectLevel);
    });
    this.setState({ levelSelectAll: selectLevel, levelClickFlag }, () => {
      const { setFieldsValue } = this.props.form;
      const ansLevel = this.strMapToArr(levelClickFlag).join('');
      setFieldsValue({ ansLevel });
    });
  ...
  handleSubmit = (type) => {
    ...
    const { questionClickFlag, levelClickFlag } = this.state;

    const questionAnsStr = this.strMapToArr(questionClickFlag).sort().join('');
    // const levelStr = this.strMapToArr(levelClickFlag).join(',');
    setFieldsValue({
      ...
      if (!err) {
        ...
        this.questionAttributeForm.props.form.validateFieldsAndScroll((errRef, valuesRef) => {
          if (!errRef) {
            ...
            // 试题分类
            // const exerciseClassifyCode = values.exerciseClassify[values.exerciseClassify.length - 1];
            // const exerciseClassifyParentCode = values.exerciseClassify[0];
            // let exerciseClassifyObj = [];
            // exerciseClassifyObj = exerciseClassifyList.filter((item) => {
            //   return (item.value == exerciseClassifyParentCode);
            // });
            // if (exerciseClassifyCode.length > 4) {
            //   exerciseClassifyObj.map((item) => {
            //     if (item.children && item.children.length > 0) {
            //       exerciseClassifyObj = item.children.filter((itemChildren) => {
            //         return (itemChildren.value == exerciseClassifyCode);
            //       });
            //     }
            //   });
            // }
            const questionItem = {
              ...
              // exerciseClassifyCode,
              // exerciseClassifyName: exerciseClassifyObj[0] ? exerciseClassifyObj[0].label : '',
  ...
  render() {
    ...
    const { levelClickFlag, questionClickFlag, testType } = this.state;
    ...
    const formQuestionAnItems = keys.map((k, index) => {
      const answerClassName = className({
        questionAnItem: true,
        active: questionClickFlag.get(formatOption(index))
      });
      return (
        <div
